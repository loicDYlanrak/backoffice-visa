...
public class ScanController {
    ...
    public ModelAndView showDocumentsAScanner(@PathVariable Long idDemande) {
        ...
        List<ScanFichierService.DocumentUploadStatus> documents = scanFichierService
                .getDocumentsAvecStatutUpload(idDemande);
    ...
    /**
     * Valide les scans et change le statut de la demande après vérification de tous
     * les uploads
     * 
     * @param idDemande          ID de la demande
    ...
    public RedirectView validerScansEtChangerStatus(
        ...
        try {
            ...
            // Changer le statut de "cree" (1) à "scanner" (2) - à adapter selon votre
            // mapping
            // Supposons que:
            // 1 = Créée
    ...
    public String pageScan(@PathVariable Long id, Model model) {
        ...
        List<ScanFichierService.DocumentUploadStatus> docsAvecStatut = scanFichierService
                .getDocumentsAvecStatutUpload(id);
        ...
        Optional<StatutDemandeEntity> dernierStatutOpt = statutDemandeRepository
                .findFirstByDemandeIdOrderByIdDesc(demande.getId());

        boolean dejaAuStatut10 = dernierStatutOpt.isPresent() && dernierStatutOpt.get().getStatut() == 10;
        ...
        if (!dejaAuStatut10) {
            ...
            System.out.println("Nouveau statut 10 enregistré.");
        } else {
            System.out.println("La demande est déjà au statut 10. Enregistrement annulé.");
        }
    ...
    public String uploadDocument(@RequestParam("demandeId") Long demandeId,
        ...
        try {
            ...
            // On attrape vos exceptions personnalisées (Fichier vide, type non autorisé,
            // etc.)
    ...
    @PostMapping("/finaliser-scan")
    public String finaliserScan(@RequestParam("demandeId") Long demandeId,
            RedirectAttributes redirectAttributes) {
        try {
            if (!scanFichierService.verifierTousDocumentsUploades(demandeId)) {
                redirectAttributes.addFlashAttribute("errorMessage",
                        "Tous les documents requis doivent être uploadés avant de finaliser");
                return "redirect:/demande/scanner/" + demandeId;
            }

            DemandeEntity demande = demandeService.findById(demandeId.intValue());
            if (demande == null) {
                redirectAttributes.addFlashAttribute("errorMessage", "Demande non trouvée");
            ...
            redirectAttributes.addFlashAttribute("successMessage",
                    "Scan finalisé avec succès !");
            return "redirect:/demande/liste";
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("errorMessage",
                    "Erreur lors de la finalisation du scan: " + e.getMessage());
            return "redirect:/demande/scanner/" + demandeId;
        }
    }
}